 ...
 *
 *     Borland C++ 5.5.
 *     bcc32 -DWIN32 -c wots.c sha256.c
 *     bcc32 -DWIN32 txbot.c wots.obj sha256.obj
 *
 *     Unix-like:
 *     cc -DUNIXLIKE -DLONG64 -c sha256.c wots.c
 *     cc -o txbot -DUNIXLIKE -DLONG64 txbot.c wots.o sha256.o
 *                                   ^
...
bal:
   ...
   for(qflag = 0 ;; Nextcore++) {
      ...
      if(cmp64(tx.send_total, Zeros) != 0 && cmp64(q[qptr].balance, Cblocknum) != 0) {
         /* We found an address with a balance, and the block has changed. */
         put64(s, tx.send_total);  /* save total amount in addr */
         sleep(10);
         break;
      }
